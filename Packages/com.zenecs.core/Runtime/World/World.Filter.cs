...
namespace ZenECS.Core
{
    public sealed partial class World
    {
        ...
        public readonly struct Filter
        {
            ...
            { withAll = wa; withoutAll = wo; withAny = wan; withoutAny = won; }
            ...
            public readonly struct Builder
            {
                ...
                private static List<List<Type>> AppendBucket(List<List<Type>> list, Type[] types)
                {
                    ...
                    foreach (var t in types) if (t != null) b.Add(t);
        ...
        internal static FilterKey MakeKey(in Filter f)
        {
            unchecked
            {
                ...
                void Mix(Type t) { h ^= (ulong)t.GetHashCode(); h *= 1099511628211ul; }
        ...
        internal ResolvedFilter ResolveFilter(in Filter f)
        {
            ...
            {
                withAll = ToPools(f.withAll),
                withoutAll = ToPools(f.withoutAll) ?? Array.Empty<IComponentPool>(),
                withAny = ToPoolBuckets(f.withAny),
                withoutAny = ToPoolBuckets(f.withoutAny),
            };
            if (rf.withAll == null) { rf.withAll = Array.Empty<IComponentPool>(); rf.withAny = Array.Empty<IComponentPool[]>(); } // 공집합
        ...
        internal static bool MeetsFilter(int id, ResolvedFilter r)
        {
            // WithAll: 모두 있어야
            var wa = r.withAll;
            for (int i = 0; i < wa.Length; i++) if (!wa[i].Has(id)) return false;

            // WithoutAll: 하나라도 있으면 탈락
            var wo = r.withoutAll;
            for (int i = 0; i < wo.Length; i++) if (wo[i].Has(id)) return false;

            // WithAny: 각 버킷마다 "적어도 하나 존재"해야 통과
            var wan = r.withAny;
            for (int b = 0; b < wan.Length; b++)
            {
                var bucket = wan[b]; bool any = false;
                for (int i = 0; i < bucket.Length; i++) if (bucket[i] != null && bucket[i].Has(id)) { any = true; break; }
                if (!any) return false;
            }
            ...
            {
                var bucket = won[b];
                for (int i = 0; i < bucket.Length; i++) if (bucket[i] != null && bucket[i].Has(id)) return false;
            }
